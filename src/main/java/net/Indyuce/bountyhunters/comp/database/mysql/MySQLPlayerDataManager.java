...
public class MySQLPlayerDataManager extends PlayerDataManager {
    ...
    public void loadData(PlayerData data) {
        try {
            ...
            /*
             * Quotes are now replaced by bounty animations but changing that
             * would corrupt all previous player data so it's voluntarily kept that way
             */
            String quoteFormat = set.getString("current_quote");
            if (quoteFormat != null)
                try {
                    Validate.isTrue(levelManager.hasAnimation(quoteFormat), "Could not load animation from " + data.getOfflinePlayer().getName());
                    data.setAnimation(levelManager.getAnimation(quoteFormat));
                } catch (IllegalArgumentException exception) {
                    data.log(exception.getMessage());
                }
