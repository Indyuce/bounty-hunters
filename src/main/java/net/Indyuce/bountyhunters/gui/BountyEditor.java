...
public class BountyEditor extends PluginInventory {
    ...
    public @NotNull Inventory getInventory() {
        ...
        if (bounty.getContributors().size() == 0) {
            ...
            ItemStack noContrib = new ItemStack(Material.RED_STAINED_GLASS_PANE);
            ...
            for (int j = 0; j < slots.length; j++) {
                ...
                meta.setLore(lore);
                meta.getPersistentDataContainer().set(CONTRIBUTOR_TAG_PATH, PersistentDataType.STRING, contributor.getUniqueId().toString());
    ...
    public void whenClicked(ItemStack item, InventoryAction action, int slot) {
        ...
        final @Nullable String tag = item.getItemMeta().getPersistentDataContainer().get(CONTRIBUTOR_TAG_PATH, PersistentDataType.STRING);
        if (tag.equals(""))
            return;
