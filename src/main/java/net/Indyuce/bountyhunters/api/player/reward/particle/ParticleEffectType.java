...
public enum ParticleEffectType {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (ti++ > 20) {
                    ...
                    loc.getWorld().playSound(loc, VersionSound.ENTITY_SHEEP_DEATH.toSound(), 1, 1);
    ...
    METEOR((loc, target) -> {
        ...
        for (int k = 0; k < 3; k++)
            new BukkitRunnable() {
                int ti = 0;
                Location source = loc.clone().add(5 * Math.cos(Utils.random.nextDouble() * 2 * Math.PI), 20, 5 * Math.sin(Utils.random.nextDouble() * 2 * Math.PI));
                Vector vec = loc.clone().subtract(source).toVector().multiply(1. / 30.);
                ...
                public void run() {
                    ...
                    if (ti >= 30) {
                        source.getWorld().playSound(source, VersionSound.ENTITY_GENERIC_EXPLODE.toSound(), 3, 1);
    ...
    private ParticleEffectType(ParticleEffect handler) {
